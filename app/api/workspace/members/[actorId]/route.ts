...
export async function GET(request: Request, context: { readonly params: Promise<{ readonly actorId: string }> }) {
  const { actorId } = await context.params;
  return createIdentityMemberRouteHandlers(openIdentityMemberServerApplication).details(request, actorId);
}
