...
export async function PATCH(request: Request, context: { readonly params: Promise<{ readonly id: string }> }) { const { id } = await context.params; return createCatalogReferenceDataRouteHandlers(openCatalogReferenceDataServerApplication).updateDepartment(request, id); }
