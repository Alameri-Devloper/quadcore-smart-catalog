...
type Context = { params: Promise<{ branchId: string; productId: string }> };

export const GET = async (request: Request, context: Context) => {
  const { branchId, productId } = await context.params;
  return createBranchProductRouteHandlers(openBranchProductServerApplication)
    .getBranchPricingManagement(request, branchId, productId);
};
