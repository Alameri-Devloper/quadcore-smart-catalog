...
export interface DirectProductShareProps {
  ...
  readonly canShareWholesale: boolean;
  readonly apiClient?: DirectProductShareApiClientPort;
  readonly deviceShare?: DeviceSharePort;
}

export function DirectProductShare({ productId, branchId, locale, canShareWholesale, apiClient, deviceShare }: DirectProductShareProps) {
  ...
  const [priceMode, setPriceMode] = useState<DirectSharePriceMode>("Retail");
  const effectivePriceMode: DirectSharePriceMode = canShareWholesale ? priceMode : "Retail";
  ...
  const [outcomeState, setOutcome] = useState<{ readonly requestKey: string; readonly value: DeviceShareOutcome | "PrepareFailed" } | null>(null);
  ...
  const prepare = async () => {
    ...
    setPreparing(false);
    if (!result.ok) { setOutcome({ requestKey: preparingKey, value: "PrepareFailed" }); return; }
  ...
  const message = preparing ? language.preparing : outcome === "Shared" ? language.shared : outcome === "Copied" ? language.copied : outcome === "Cancelled" ? language.cancelled : outcome === "PrepareFailed" ? language.prepareFailed : outcome === "Failed" || outcome === "Unsupported" ? language.failed : prepared ? language.prepared : "";
  ...
  return <section className="direct-share" dir={locale === "ar" ? "rtl" : "ltr"} aria-labelledby={`direct-share-${productId}`} aria-busy={preparing}>
    ...
    {canShareWholesale ? <fieldset className="direct-share__modes" disabled={preparing}>
      <legend className="sr-only">{language.share}</legend>
      {(["Retail", "Wholesale"] as const).map((mode) => <label key={mode}>
    ...
    </fieldset> : <p className="direct-share__selected-mode">{language.retail}</p>}
