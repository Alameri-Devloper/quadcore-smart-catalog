...
import { ProductMediaFinalKey, ProductMediaStorageRootKey } from "../../../media/domain/product-media-keys";
...
import type { DirectShareMediaReaderPort, DirectShareMediaReadResult } from "../../ports/direct-product-share-repository.port";

export class LocalDirectShareMediaReaderAdapter implements DirectShareMediaReaderPort {
  constructor(private readonly configuredRoot = process.env.QSC_MEDIA_ROOT) {}

  async read(input: Parameters<DirectShareMediaReaderPort["read"]>[0]): Promise<DirectShareMediaReadResult> {
    try {
      ...
      await ProductMediaRoot.rehydrate({
