...
export const ProductEntryReviewService = {
  createViewModel(input: CreateReviewInput): ProductEntryReviewViewModel {
    ...
    const validImages = values.images.filter((image) => image.operationType !== "Remove" && (image.operationType === null || (image.sourceAvailability === "AvailableInCurrentSession" && image.hashStatus === "Ready")));
    const hasValidMain = validImages.some((image) => image.isPrimary);
    const hasReselection = values.images.some((image) => image.sourceAvailability === "RequiresReselection");
    ...
    const commercialResults = [values.retailPrice !== null && Number.isFinite(values.retailPrice) && values.retailPrice >= 0, isApprovedProductEntryCurrency(values.currency), isApprovedProductCondition(values.condition), isApprovedProductAvailability(values.availabilityStatus), typeof values.isFeatured === "boolean", values.publicationIntent === "SaveAsDraft" || values.publicationIntent === "PublishWhenReady"];
    ...
    const categories: ProductEntryReviewScoreCategory[] = [
      ...
      { label: "Presentation Readiness", score: scoreRules(PRODUCT_ENTRY_REVIEW_POLICY.score.presentation.rules, [values.publicationIntent === "PublishWhenReady", Boolean(values.productCode.trim()), optionalFields.length === 0 || optionalCompleted === optionalFields.length, warnings.filter((warning) => warning.section !== "Commercial Details" || warning.problem !== "Wholesale Price is not entered.").length === 0]), maximum: 10, explanation: "Based on publication intent, Product Code, optional specifications, and presentation warnings." },
    ];
    ...
    const customerRequirements = readyToSave && values.publicationIntent === "PublishWhenReady" && Boolean(values.productName.trim()) && commercialResults[0] && commercialResults[1] && requiredCompleted === requiredFields.length && validImages.length > 0 && hasValidMain && !hasReselection;
    ...
    return {
      ...
      commercial: { values: [{ label: "Product Name", value: values.productName || "Not entered" }, values.productCode ? { label: "Product Code", value: values.productCode } : null, { label: "Retail Price", value: values.retailPrice === null ? "Not entered" : `${values.retailPrice} ${values.currency}` }, values.wholesalePrice === null ? null : { label: "Wholesale Price", value: `${values.wholesalePrice} ${values.currency}` }, { label: "Currency", value: values.currency || "Not entered" }, { label: "Condition", value: PRODUCT_ENTRY_CONDITION_OPTIONS.find((option) => option.value === values.condition)?.label ?? "Not entered" }, { label: "Availability", value: PRODUCT_ENTRY_AVAILABILITY_OPTIONS.find((option) => option.value === values.availabilityStatus)?.label ?? "Not entered" }, { label: "Featured", value: values.isFeatured ? "Yes" : "No" }, { label: "Publication intent", value: values.publicationIntent === "PublishWhenReady" ? "Publish when ready" : "Save as draft" }].filter((item): item is { label: string; value: string } => item !== null) },
      images: { count: validImages.length, mainStatus: validImages.length === 0 ? "No images added" : hasValidMain ? "Main Product Image confirmed" : "Main Product Image needs attention", values: [...values.images].filter((image) => image.operationType !== "Remove").sort((a, b) => a.sortOrder - b.sortOrder).map((image) => ({ label: `${image.sortOrder}. ${image.fileName ?? image.mediaId ?? "Server image"}`, value: `${image.operationType ?? "Existing"} · ${image.sourceAvailability} · ${image.hashStatus}` })) },
