    ...
    <div>
      ...
      <section className="mt-8" aria-labelledby="commercial-identity-heading">
        ...
        <div className="mt-4 grid grid-cols-1 gap-5 md:grid-cols-2">
          <div className="md:col-span-2">
            ...
            <input aria-describedby={fieldDescription("productName")} aria-invalid={issuesByField.has("productName")} className={inputClass} id="productName" onBlur={() => void setValue("productName", values.productName.trim())} onChange={(event) => void setValue("productName", event.target.value)} required type="text" value={values.productName} />
      ...
      <details className="mt-9 rounded-2xl border border-slate-200 bg-slate-50 p-5">
        ...
        <div className="mt-5 grid grid-cols-1 gap-6 md:grid-cols-2">
          <fieldset><legend className="text-sm font-semibold text-slate-900">Feature this product <span className="font-normal text-slate-600">(Optional)</span></legend><div className="mt-2 flex gap-3">{([true, false] as const).map((decision) => <label className="flex min-h-12 flex-1 cursor-pointer items-center gap-3 rounded-xl border border-slate-300 bg-white px-4 focus-within:ring-4 focus-within:ring-blue-200" key={String(decision)}><input checked={values.isFeatured === decision} name="isFeatured" onChange={() => void setValue("isFeatured", decision)} type="radio" /><span>{decision ? "Yes" : "No"}</span></label>)}</div></fieldset>
