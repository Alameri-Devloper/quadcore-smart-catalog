      ...
      <div className="mt-6 rounded-2xl border border-dashed border-blue-300 bg-blue-50 p-5">
        <label className="inline-flex min-h-12 cursor-pointer items-center rounded-xl bg-blue-600 px-5 text-sm font-semibold text-white focus-within:ring-4 focus-within:ring-blue-200">
          ...
          <input
            ...
            multiple
            onChange={(event) => {
              const result = productEntryImagesService.createReferences(Array.from(event.target.files ?? []), imagesRef.current);
              ...
              if (result.images.length > 0) {
                ...
                updateImages(next);
                setAnnouncement(`${result.images.length} image${result.images.length === 1 ? "" : "s"} added.`);
              }
              event.target.value = "";
            }}
            type="file"
            ...
            return (
              <li className="rounded-2xl border border-slate-200 bg-slate-50 p-4 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue-200" key={image.id} ref={(node) => { if (node) cardRefs.current.set(image.id, node); else cardRefs.current.delete(image.id); }} tabIndex={-1}>
                <div className="relative mx-auto aspect-square w-full max-w-64 overflow-hidden rounded-xl border border-slate-200 bg-white">
                  {previewUnavailable ? <div className="flex h-full items-center justify-center p-4 text-center text-sm text-slate-600">This image could not be previewed. Select it again or remove it.</div> : <Image alt={`${image.fileName} selected product image preview`} className="object-contain" fill onError={() => setUnavailablePreviews((current) => new Set(current).add(previewKey))} sizes="(min-width: 1280px) 20rem, (min-width: 768px) 50vw, 100vw" src={previewUrl} unoptimized />}
                ...
                <section className="mt-5 rounded-xl border border-slate-200 bg-white p-4" aria-labelledby={`background-${image.id}`}>
                  ...
                  <p className="mt-3 text-xs leading-5 text-slate-600">White-background processing will be available through a replaceable image-processing service.</p>
                  <div className="mt-4 flex flex-wrap gap-2">
                    ...
                    {image.processingStatus !== "processing" ? <button className={buttonClass} onClick={() => replaceImage(productEntryImagesService.skip(image))} type="button">Skip Processing</button> : null}
                  </div>
