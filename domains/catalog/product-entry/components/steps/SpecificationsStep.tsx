        ...
        <div className="mt-7 grid grid-cols-1 gap-x-6 gap-y-6 md:grid-cols-2 xl:grid-cols-3">
          {fields.map((field) => {
            ...
            if (field.fieldType === "boolean") {
              return <fieldset aria-describedby={describedBy} className={shellClass} key={field.specificationFieldId}><legend className="text-sm font-semibold text-slate-900">{field.label} <span className="font-normal text-slate-600">({field.required ? "Required" : "Optional"})</span></legend><div className="mt-3 flex gap-3">{([true, false] as const).map((decision) => <label className="flex min-h-12 flex-1 cursor-pointer items-center gap-3 rounded-xl border border-slate-300 bg-white px-4 focus-within:ring-4 focus-within:ring-blue-200" key={String(decision)}><input checked={value === decision} name={inputId} onChange={() => updateValue(field.specificationFieldId, decision)} required={field.required} type="radio" /><span>{decision ? "Yes" : "No"}</span></label>)}</div>{error ? <p className="mt-2 text-sm font-medium text-red-700" id={errorId}>{error}</p> : null}</fieldset>;
            }

            const selectedOptionId = field.fieldType === "select"
              ? (field.options.find((option) => Object.is(option.value, value))?.optionId ?? "")
              : "";
            return <div className={shellClass} key={field.specificationFieldId}><label className="text-sm font-semibold text-slate-900" htmlFor={inputId}>{field.label} <span className="font-normal text-slate-600">({field.required ? "Required" : "Optional"})</span></label>{field.fieldType === "select" ? <select aria-describedby={describedBy} aria-invalid={Boolean(error)} className="mt-2 min-h-12 w-full rounded-xl border border-slate-300 bg-white px-3 text-base focus:border-blue-600 focus:outline-none focus:ring-4 focus:ring-blue-100 disabled:bg-slate-100" disabled={Boolean(field.configurationError)} id={inputId} onChange={(event) => updateValue(field.specificationFieldId, field.options.find((option) => option.optionId === event.target.value)?.value)} required={field.required} value={selectedOptionId}><option value="">Choose an option</option>{field.options.map((option) => <option key={option.optionId} value={option.optionId}>{option.label}</option>)}</select> : <input aria-describedby={describedBy} aria-invalid={Boolean(error)} className="mt-2 min-h-12 w-full rounded-xl border border-slate-300 bg-white px-4 text-base focus:border-blue-600 focus:outline-none focus:ring-4 focus:ring-blue-100" id={inputId} onChange={(event) => updateValue(field.specificationFieldId, field.fieldType === "number" ? (event.target.value === "" ? undefined : event.target.valueAsNumber) : event.target.value)} required={field.required} type={field.fieldType === "number" ? "number" : "text"} value={typeof value === (field.fieldType === "number" ? "number" : "string") ? String(value) : ""} />}{field.helpText ? <p className="mt-2 text-sm text-slate-600" id={helpId}>{field.helpText}</p> : null}{error ? <p className="mt-2 text-sm font-medium text-red-700" id={errorId}>{error}</p> : null}</div>;
          })}
        </div>
      ) : null}
