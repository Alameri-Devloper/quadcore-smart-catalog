...
export const catalogProducts = pgTable(
  ...
  (table) => [
    ...
    index("catalog_products_query_newest_idx").on(table.workspaceId, table.lifecycleState, table.createdAt, table.productId),
    index("catalog_products_query_name_idx").on(table.workspaceId, table.lifecycleState, sql`lower(coalesce(${table.productName}, ''))`, table.productId),
    index("catalog_products_query_retail_idx").on(table.workspaceId, table.lifecycleState, table.retailPriceCurrency, table.retailPriceMinor, table.productId),
    index("catalog_products_name_trgm_idx").using("gin", sql`lower(coalesce(${table.productName}, '')) gin_trgm_ops`),
    index("catalog_products_code_trgm_idx").using("gin", sql`lower(coalesce(${table.productCode}, '')) gin_trgm_ops`),
    index("catalog_products_search_vector_idx").using("gin", sql`to_tsvector('simple', coalesce(${table.productName}, '') || ' ' || coalesce(${table.productCode}, ''))`),
...
export const catalogBranchProductListings = pgTable("catalog_branch_product_listings", {
  ...
  index("catalog_branch_product_listings_query_idx").on(table.workspaceId, table.branchId, table.listingStatus, table.productId),
