...
import type { CatalogSearchInput, OperationalProductSearchInput } from "../../application/catalog-query.use-cases";
...
const operationalAllowed = new Set(["purpose", "q", "branchId", "cursor", "limit"]);
const parseOperationalSearch = (request: Request): OperationalProductSearchInput | null => { const params = new URL(request.url).searchParams; for (const key of params.keys()) if (!operationalAllowed.has(key) || params.getAll(key).length !== 1) return null; const purpose=params.get("purpose"); if(!purpose)return null; const rawLimit=params.get("limit"); if(rawLimit!==null&&!/^[1-9][0-9]*$/u.test(rawLimit))return null; return {purpose,q:params.get("q")??undefined,branchId:params.get("branchId")??undefined,cursor:params.get("cursor")??undefined,limit:rawLimit===null?undefined:Number(rawLimit)}; };
...
export const createCatalogQueryRouteHandlers = (open: Open) => ({
  ...
  operationalSearch: (request: Request) => withApp(open, request, async (app, context) => { const input=parseOperationalSearch(request); return input ? response(await app.operationalSearch.execute({context,input})) : json({type:"InvalidQuery"},400); }),
